...
export class ItemSaleReportComponent implements OnInit {
  ...
  onPageNoChange = (event) => {
    ...
    this.getItemSaleDetails(UIConstant.SALE_TYPE, 0, '', '', fromDate, toDate, '', this.page, this.pageNo)
    ...
    this._commonService.itemSalePurchaseReportDetails(type, LedgerId, CategoryId, ItemId, FromDate, ToDate, BatchNo, Pagenumber, Size).subscribe(data => {
      if (data.Code === UIConstant.THOUSAND) {
        ...
        if (data.Data.SummarizeDetails.length > 0) {
          ...
          this.MainExcelSummary =[this.TotalQty , this.totalDiscountAmt,this.totalTaxAmount , this.SubTotalAmount ]
  ...
  export() {
    if (this.mainDataEcxel.length > 0) {
      this.excelService.generateExcel(this.orgDetails.OrganizationDetails[0].OrgName, this.orgDetails.AddressDetails[0].CityName + ' ' + this.orgDetails.AddressDetails[0].StateName + ' ' + this.orgDetails.AddressDetails[0].CountryName, this.ExcelHeaders, this.mainDataEcxel,
       'Item Sale Report', this.setFromadate, this.setTodate,this.ExcelSummary)
