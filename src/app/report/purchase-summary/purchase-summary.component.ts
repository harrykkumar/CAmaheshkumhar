...
export class PurchaseSummaryReportComponent implements OnInit, AfterViewInit {
  cashBook: any = {}
  ledgerItemList: any = [];
   ...
   // alert(this.model.fromDatevalue)
    ...
    this._commonService.getReporSalePurchaserSummary(data).pipe(
      ...
      if (response.Code === UIConstant.THOUSAND && response.Data && response.Data.Itemstockdetails.length > 0) {
        ...
        // console.log( this.orgDetails ,'getss')
        let value = []
        ...
        // this.topToBottomSummary(response.Data.TopRightSummary)
  ...
  //   this.MonthlyData.forEach(del => {
  //     let index =  element.findIndex(n => n.Month === del.month)
  //     if (index >= 0) {
  //            obj[del.month] = element[0].Balance   
  //     }
  //     else{
