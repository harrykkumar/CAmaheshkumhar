...
declare var $: any
@Component({
  selector: 'app-outstanding-receiables-report',
  templateUrl: './outstanding-receiables-report.component.html',
  styleUrls: ['./outstanding-receiables-report.component.css']
})
export class OutStandingReceiablesReportComponent implements OnInit, AfterViewInit {
  ...
  getLedgerItemList = () => {
    this._commonService.getSalePurchaseUtilityItems().pipe(
      ...
      map((data: any) => {
        return _.map(data.Data.Customers, (element) => {
          return {
            id: element.Id,
            text: element.Name
          }
