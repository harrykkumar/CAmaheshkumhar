...
export class PurchaseRegisterComponent implements OnInit, AfterViewInit {
  ...
  getPurchaseRegisterData = () => {
    ...
    this._commonService.getSalePurchaseRegisterReport(data).pipe(
      ...
      if (response.Code === UIConstant.THOUSAND && response.Data && response.Data.Itemstockdetails.length > 0) {
        this.purchaseRegister = response.Data;
        this.getValueFalg = false
        this.mainDataExcel = []
        this.ExcelHeaders = ["SNo", "Months", "Total Purchases", "Purchase Return", "Net Purchases"]
  ...
  exportExcel() {
    if (this.mainDataExcel.length > 0) {
      this.excelService.generateExcel(this.purchaseRegister.OrganizationDetails[0].OrgName,
        this.purchaseRegister.AddressDetails[0].CityName + ' ' +
        this.purchaseRegister.AddressDetails[0].StateName + ' ' + this.purchaseRegister.AddressDetails[0].CountryName, this.ExcelHeaders,
        this.mainDataExcel, 'Purchase Register Report', this.model.fromDatevalue, this.model.toDateValue,[])
