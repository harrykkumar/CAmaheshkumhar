...
@Component({
  selector: 'vendor-rates-search',
  templateUrl: './vendor-rates-search.component.html'
})
export class VendorRatesSearchComponent {
  ...
  constructor (private formBuilder: FormBuilder, private _ledgerServices: VendorServices, private settings: Settings,
    private gs: GlobalService, private _ts: ToastrCustomService, private _vrs: VendorRatesService, private _po: PackedOrdersService) {}
  ...
  getLedgerList () {
    ...
    this.gs.manipulateResponse(this._ledgerServices.getVendor(4, '')).subscribe(data => {
  ...
  search () {
    if (this.searchForm.valid) {
      ...
      this._vrs.setSearchQueryParamsStr(queryStr)
    }
  }
