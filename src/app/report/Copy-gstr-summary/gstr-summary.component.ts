...
export class GstrSummaryComponent implements OnInit {
  ...
  ngOnInit() {
    this.mainDataExcel=[]
    this.ExcelHeaders = [ "S.No.","Bill Date", "Invoice No.", "Ledger Name", "State",
    "Item Name", "	MRP Rate", "HSN No.","Qty","Sale Rate","Unit","Taxable Amount"]
  ...
  exportExcel() {
    if (this.mainDataExcel.length > 0) {
      //this.excelService.generateExcel(this.DayBook.OrganizationDetails[0].OrgName, this.DayBook.AddressDetails[0].CityName + ' ' + this.DayBook.AddressDetails[0].StateName + ' ' + this.DayBook.AddressDetails[0].CountryName, this.ExcelHeaders, this.mainDataExcel, 'GSTR-Summary', this.model.fromDatevalue, this.model.toDateValue,[])
    }
  }
}
