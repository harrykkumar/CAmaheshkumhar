...
import { Component, ViewChild, QueryList, ViewChildren, Renderer2, ElementRef, Output, EventEmitter } from '@angular/core';
import { UIConstant } from 'src/app/shared/constants/ui-constant'
import { Select2OptionData, Select2Component } from 'ng2-select2'
import { POPurchaseService } from './po-purchase.service'
...
declare const _: any
@Component({
  selector: 'po-purchase-add',
  templateUrl: './po-purchase.component.html',
  styleUrls: ['./po-purchase.component.css']
})
export class PoPurchaseComponent {
  ...
  constructor(private commonService: CommonService,
    private _pops: POPurchaseService,
    ...
    this.onDestroy$.push(this.commonService.openDiscountMasterStatus().subscribe((data: AddCust) => {
   ...
   this.onDestroy$.push(this.commonService.getCatImportAddStatus.subscribe(() => {
        this.getAllCategoriesOnImport()
      }
    ))

    this.onDestroy$.push(this.commonService.getAttributeStatus().subscribe(data => {
    ...
    this.onDestroy$.push(this.commonService.getledgerCretionStatus().subscribe(data => {
    ...
    this.onDestroy$.push(this.commonService.getCategoryStatus().subscribe((data: AddCust) => {
        if (data.id && data.name) {
    ...
    this.onDestroy$.push(this._pops.attributesData$.subscribe(data => {
    ...
    let _self = this
    this.onDestroy$.push(this._pops.addressData$.subscribe(data => {
        if (data.data) {
          _self.AddressData = Object.assign([], data.data)
          console.log('addresses : ', _self.AddressData)
          _self.addressValue = +_self.spUtilityData.AddressId
        }
      }
    ))

    this.onDestroy$.push(this.commonService.getAddressStatus().subscribe((data: AddCust) => {
        if (data.id && data.name) {
    ...
    this.onDestroy$.push(this.commonService.getCompositeUnitStatus().subscribe((data: AddCust) => {
        if (data.id && data.name) {
    ...
    this.onDestroy$.push(this.commonService.getUnitStatus().subscribe(
      (data: AddCust) => {
        if (data.id && data.name) {
          let newData = Object.assign([], this.subUnitsData)
          newData.push({ id: +data.id, text: data.name })
    ...
    this.onDestroy$.push(this.commonService.getItemMasterStatus().subscribe(
      (data: AddCust) => {
    ...
    this.onDestroy$.push(this.commonService.getTaxStatus().subscribe((data: AddCust) => {
        if (data.id && data.name) {
  ...
  getAllCategoriesOnImport() {
    this.onDestroy$.push(this._catagoryservices.GetCatagoryDetail('').subscribe(data => {
  ...
  createItems(ItemTransactions) {
    ItemTransactions.forEach((element) => {
      ...
      let attributesBefore = this.itemAttributesOthers.filter((attr) =>  attr.ItemTransId === element.Id)
      console.log(attributesBefore)
      ...
      this.itemAttributeTrans = attributesBefore
  ...
  other: any = {}
  createOther(others) {
    this.model['VendorName'] = others.VendorName
    this.CurrentDate = this.gs.utcToClientDateFormat(others.CurrentDate, this.clientDateFormat)
    this.model['LedgerId'] = others.LedgerId
    this.model['outStandingBalance'] = (others.OpeningAmount).toFixed(this.noOfDecimalPoint)
    this.model['setCRDR'] = others.Crdr === 0 ? 'Cr' : 'Dr';
  ...
  getAllCategories(categoryName, categoryId, isAddNew) {
    this.onDestroy$.push(this.commonService.getAllCategories().subscribe(
      data => {
  ...
  getAllAddresses(vendorId) {
    this.onDestroy$.push(this._pops.getAllAddresses(vendorId).subscribe(data => {
      if (data.Code === UIConstant.THOUSAND && data.Data) {
        if (data.Data.AddressDetails && data.Data.AddressDetails) {
          ...
          this._pops.createAddress(data.Data.AddressDetails)
        }
  ...
  getNewBillNo() {
    if (!this.editMode) {
      if (+this.OrgId > 0 && this.CurrentDate) {
        ...
        this.onDestroy$.push(this._pops.getNewBillNoPurchase(+this.OrgId, newBillDate, type, 'Purchase').subscribe(
          data => {
            if (data.Code === UIConstant.THOUSAND && data.Data) {
  ...
  openModal(idStr) {
    ...
    this.initComp()
    this.getSetUpModules((JSON.parse(this.settings.moduleSettings).settings))
    $('#po_purchase_modal').modal(UIConstant.MODEL_SHOW)
    this.getFormDependency()
    this.onDestroy$.push(this._po.getPOItemsForPurchase(idStr).subscribe(
      (data) => {
        this.loading = false
        console.log('purchase data : ', data)
        this.dataForEdit = data
        this.spUtilityData = data.PurchaseOrderDetails[0]
        this.allItems = [...data.Items]
        this.allAddressData = data.AddressDetails
        this.allCategories = [...data.ItemCategorys]
        this._pops.createAddress(data.AddressDetails)
        if (data.AttributeValueResponses.length > 0) {
          this._pops.generateAttributes(data.AttributeValueResponses)
        }
        if (data && data.ItemCategorys && data.ItemCategorys.length > 0) {
          this.getCatagoryDetail(data.ItemCategorys)
        }
        this.itemData = this.getList(data.Items, 'Name', 'Item', true)
        this.paymentModesData = this.getList(data.PaymentModes, 'PayModeName', 'Payment Mode', false)
        this.godownsData = this.getList(data.Godowns, 'Name', 'Godown', false)
        this.subUnitsData = this.getList(data.SubUnits, 'Name', 'Unit', true)
        this.taxSlabsData = this.getList(data.TaxSlabs, 'Slab', 'Tax Slab', true)
        this.currencyData = this.getList(data.Currencies, 'Name', 'Currency', false)
        this.convertToCurrencyData = this.currencyData
        this.chargesData = this.getList(data.LedgerCharges, 'Name', 'Charge', true)
        if (this.godownsData.length > 0) {
          this.GodownId = +this.godownsData[1].id
          this.godownValue = +this.godownsData[1].id
        }
        this.defaultCurrency = this.currencyData[1].text
        this.currencyValues = [
          { id: '0', symbol: '%' },
          { id: '1', symbol: this.defaultCurrency }
        ]
        this.convertToCurrencyData = [...this.currencyData]
        this.CurrencyId = +this.currencyData[1].id
        this.currencyValue = +this.currencyData[1].id
        this.ConvertToCurrencyId = +this.convertToCurrencyData[1].id
        this.convertToCurrencyValue = +this.convertToCurrencyData[1].id
        this.createForm(this.dataForEdit)
      },
      (error) => {
  ...
  getList(data, key, title, flag, id?){
    let id1 = 'Id'
    if (id) {
      id1 = id
    }
    ...
    if (flag) {
      return [{ id: 0, text: 'Select ' + title }, 
      {id: -1, text: UIConstant.ADD_NEW_OPTION}, ...list]
    } else {
      return [{ id: 0, text: 'Select ' + title }, ...list]
    }
  }

  checkForExistence: any = []
  getFormDependency() {
    this.onDestroy$.push(this.gs.manipulateResponse(this.commonService.getFormDependency(UIConstant.PURCHASE_TYPE)).subscribe(
      data => {
  ...
  getCatLevel() {
    let _self = this
    this.onDestroy$.push(this.commonService.getSettingById(SetUpIds.catLevel).subscribe(
      (data) => {
  ...
  setDueDate(setups) {
    if (setups && setups.length > 0) {
      this.model['DueDate'] = this.gs.utcToClientDateFormat(setups[0].CurrentDate, this.clientDateFormat)
    }
  }
  setBillDate() {
  ...
  checkForItems(categoryId) {
    ...
    if (categoryId > 0) {
      ...
      if (this.editItemId === -1) {
        this._pops.createItems(newItemsList)
      } else {
      ...
      this._pops.createItems(this.allItems)
    }
  }
  ...
  getItemDetail(Billdate, barcode, ItemId, CustomerId) {
    this.onDestroy$.push(this.commonService.barcodeAPI(Billdate, barcode, ItemId, CustomerId).subscribe(data => {
  ...
  calculate() {
    ...
    if (this.validDiscount) {
      ...
      if (total > 0) {
        ...
        if (this.TaxType === 0) {
          let returnTax = this._pops.taxCalculation(this.taxRates,
            this.taxSlabType,
            discountedAmount,
          ...
          if (this.taxCalInclusiveType === 1) {
            let AmountItem = +(this._pops.calcTaxableAmountType1(this.taxRates,
              this.taxSlabType,
              discountedAmount,
            ...
            let returnTax = this._pops.taxCalCulationForInclusive(this.taxRates,
              this.taxSlabType,
              this.AmountItem,
              this.isOtherState, FormConstants.PurchaseForm, this.taxSlabName)
            this.TaxAmount = +(returnTax.taxAmount).toFixed(4)
            ...
            let AmountItem = +(this._pops.calcTaxableAmountType2(this.taxRates,
              this.taxSlabType,
              total,
              this.isOtherState)).toFixed(4)
            if ('' + this.DiscountType === '0') {
            ...
            let returnTax = this._pops.taxCalCulationForInclusiveType2(this.taxRates,
              this.taxSlabType,
              this.AmountItem,
              this.isOtherState, FormConstants.PurchaseForm, this.taxSlabName)
            this.TaxAmount = +(returnTax.taxAmount).toFixed(4)
    ...
    if (this.taxChargeRates.length > 0 && +this.AmountCharge > 0) {
      if (this.TaxTypeCharge === 0) {
        let returnTax = this._pops.taxCalculation(this.taxChargeRates,
        ...
        if (this.TaxTypeCharge === 1) {
          let AmountCharge = this._pops.calcTaxableAmountType1(this.taxChargeRates,
          ...
          let returnTax = this._pops.taxCalCulationForInclusive(this.taxChargeRates,
            this.taxChargeSlabType,
            +AmountCharge,
            this.isOtherState, FormConstants.ChargeForm, this.TaxChargeName)
          this.TaxAmountCharge = +(returnTax.taxAmount).toFixed(4)
    ...
    if (this.AdditionalCharges.length > 0) {
      ...
      for (const charge of this.AdditionalCharges) {
        if (charge.TaxSlabChargeId !== 0) {
          observables.push(this._pops.getSlabData(charge.TaxSlabChargeId));
        }
      }
          ...
          data.forEach((element, index) => {
            ...
            if (+this.AdditionalCharges[index].AmountCharge > 0) {
              ...
              if (this.AdditionalCharges[index].TaxTypeCharge === 1) {
                let returnTax = this._pops.taxCalCulationForInclusive(element.Data.TaxRates,
                  taxChargeSlabType,
                ...
                let returnTax = this._pops.taxCalculation(element.Data.TaxRates,
                  taxChargeSlabType,
                  +this.AdditionalCharges[index].AmountCharge,
                  this.isOtherState, FormConstants.ChargeForm, element.Data.TaxSlabs[0].Slab)
                this.AdditionalCharges[index]['TaxAmountCharge'] = returnTax.taxAmount
  ...
  updateItemTax() {
    ...
    if (this.Items.length > 0) {
      ...
      for (const item of this.Items) {
        if (item.TaxSlabId !== 0) {
          observables.push(this._pops.getSlabData(item.TaxSlabId));
        }
      }
      this.onDestroy$.push(forkJoin(...observables).subscribe(
        data => {
  ...
  setpaymentLedgerSelect2(i, paymentId) {
    ...
    this.onDestroy$.push(this.commonService.getPaymentLedgerDetail(paymentId).subscribe(data => {
  ...
  addItemBasedOnIndustry() {
    ...
    this.Items.push({
      ...
      categoryName: this.getPattern() || this.categoryName,
  ...
  closeModal() {
    ...
    $('#po_purchase_modal').modal(UIConstant.MODEL_HIDE)
  }
  ...
  getNewCurrentDate() {
    this.onDestroy$.push(this._pops.getCurrentDate().subscribe(
      data => {
  ...
  manipulateData() {
    ...
    let valid = 1
    this.checkForFocus()
    this.onDestroy$.push(this.gs.manipulateResponse(this.commonService.checkForExistence(this.checkForExistence, dataToSend))
      .subscribe((data) => {
      ...
      () => {
        if (valid) {
          if (this.checkForValidation() && this.isValidAmount && this.validItem && this.validTransaction) {
            ...
            this.onDestroy$.push(this._pops.postPoPurchase(dataToSend).subscribe(
              data => {
                this.closeModal()
  ...
  getTaxChargeDetail(TaxSlabId) {
    this.onDestroy$.push(this._pops.getSlabData(TaxSlabId).subscribe(
      data => {
        if (data.Code === UIConstant.THOUSAND && data.Data) {
          if (this.vendorGSTType === 1) {
