...
export class ItemImportComponent implements OnDestroy {
  ...
  addToQueue () {
    ...
    if (this.isPending) {
      ...
      this.selectedItems.forEach(item => {
        for (let i = 0; i < this.masterDataCopy.length; i++) {
          if (item === this.masterDataCopy[i]['SNO']) {
            itemImport.push(this.masterDataCopy[i])
            itemImport[itemImport.length - 1]['BRANDIDS'] = this.brands
            itemImport[itemImport.length - 1]['CATEGORY'] = !isNaN(this.categoryId) ? this.categoryId : 0
            itemImport[itemImport.length - 1]['TAXID'] = !isNaN(this.taxId) ? this.taxId : 0
          }
      ...
      this.selectedItems.forEach(item => {
        for (let i = 0; i < this.masterDataCopy.length; i++) {
          if (item === this.masterDataCopy[i]['SNO']) {
            this.masterDataCopy.splice(i, 1)
          }
        }
      })
      this.masterData = JSON.parse(JSON.stringify(this.masterDataCopy))
    }
    this.selectedItems = []
  ...
  readingData () {
    ...
    fileReader.onload = (e) => {
      ...
      if (this.masterTableArray.length > 0) {
        ...
        'ISNOTDISCOUNTABLE', 'ISVOLUMEDISCOUNTAPPLY', 'ISTRADEDISCOUNTAPPLY', 'TAXRATE']
        ...
        if (resp !== '') {
          ...
          this.masterTableArray.forEach(element => {
            ...
            else if (!newRow['HSNNO'] && this.isHSNMandatory) {
  ...
  masterDataCopy: any = []
  createTaxRatesArray() {
    let newData = [{id: '0', text: 'Select Tax Rate'}]
    let taxRates = []
    this.masterData.forEach((element) => {
      taxRates.push(element.TAXRATE)
    })
    this.taxRates = _.uniq(taxRates)
    this.taxRates.forEach((elem) => {
      newData.push({
        id: elem,
        text: elem
      })
    });
    this.taxRates = [...newData]
    console.log(this.taxRates)
    if (this.taxRates.length > 1) {
      this.masterDataCopy = JSON.parse(JSON.stringify(this.masterData))
    }
  }

  filterData(evt) {
    console.log(evt.value)
    this.masterData = this.masterDataCopy.filter((element) => element.TAXRATE === evt.value)
  }
  ...
  deleteDuplicates () {
    ...
    this.duplicateTuples.forEach(item => {
      for (let i = 0; i < this.masterDataCopy.length; i++) {
        if (item['SNO'] === this.masterDataCopy[i]['SNO']) {
          this.masterDataCopy.splice(i, 1)
        }
      }
    })
