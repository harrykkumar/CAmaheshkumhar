...
export class ServiceItemComponent implements OnInit, OnDestroy {
  ...
  constructor (private _itemmasterServices: ItemmasterServices,
    ...
    this.deleteSub = this.commonService.getDeleteStatus().subscribe(
      (obj) => {
        if (obj.id && obj.type && obj.type === 'ItemService') {
  ...
  getItemMasterDetail () {
    ...
    const queryStr = this.queryStr ? this.queryStr : ''
    this.commonService.getServiceItemListData('?Strsearch=' + this.searchForm.value.searckKey + '&Page=' + this.p + '&Size=' + this.itemsPerPage + queryStr).subscribe(data => {
      console.log('item master : ', data)
      if (data.Code === UIConstant.THOUSAND) {
        console.log(data ,"service -item")
        this.itemDetail = data.Data
        this.total = this.itemDetail[0] ? this.itemDetail[0].TotalRows : 0
        setTimeout(() => {
          this.isSearching = false
  ...
  editItem (id) {
    ...
    this.commonService.openServiceItem(id, 0)
  }

  deleteItemMaster (id) {
    console.log('id : ', id)
    this.commonService.openDelete(id, 'ItemService', 'ItemService')
  }
