...
export class CustomerSearchComponent implements OnChanges {
  ...
  custTypePlaceholder: Select2Options
  ...
  custTypeSelect2: Array<Select2OptionData> = []
  ...
  getCustomerType () {
    this.custTypePlaceholder = { placeholder: 'Select CustomerType' }
    let newData = [{ id: '0', text: 'CustomerType' }]
    this.ledgerService.getCommonValues('116').subscribe(data => {
      if (data.Code === UIConstant.THOUSAND && data.Data) {
        ...
        this.custTypeSelect2 = Object.assign([], newData)
      }
    })
  }

  selectCustomerType (evt) {
    if (evt.value && evt.data.length > 0) {
      this.CustomerTypeId = +evt.value
    }
  }
  ...
  search () {
    ...
    const queryStr = '&RegistrationTypeId=' + this.RegistrationTypeId +
     '&CustomerTypeId=' + this.CustomerTypeId + '&CountryId=' +
