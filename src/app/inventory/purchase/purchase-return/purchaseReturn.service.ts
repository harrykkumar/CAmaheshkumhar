...
export class SaleDirectReturnService {
  ...
  getSaleReturnList (queryParams) {
    return this.baseService.getRequest(ApiConstant.RETURN_SALE_DIRECT + queryParams)
  }
  ...
  createReferralTypes (array) {
    let newData = [{ id: '0', text: 'Select Refferal ' }]
    array.forEach(data => {
      newData.push({
        id: data.Id,
  ...
  getSaleEditData(id): Observable<ResponseSale> {
    return this.baseService.getRequest(ApiConstant.DIRECT_SALE_EDIT_GET_API+'PurchaseReturnDetails?Id=' + id)
  }
