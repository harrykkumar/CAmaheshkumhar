...
import { PurchaseAttribute, AddCust, PurchaseReturnAdd, PurchaseTransaction, PurchaseItem } from '../../../model/sales-tracker.model'
...
@Component({
  ...
  templateUrl: './purchaseReturn-add.component.html',
  styleUrls: ['./purchaseReturn-add.component.css']
})
export class PurchaseReturnComponent {
  ...
  constructor( private _loaderService : NgxSpinnerService,private commonService: CommonService,
    ...
    this.data$ = this.commonService.getActionSaleReturnClickedStatus().subscribe(
      (action: any) => {
        console.log(action)
        if (action.type === FormConstants.Edit && action.formname === 3) {
    ...
    this.data$ = this.commonService.getActionClickedStatus().subscribe(
      (action: any) => {
        if (action.type === FormConstants.Return && action.formname === 3) {
          this.creatingForm = true
          this.editMode = false
          this.Id = +action.id
 ...
 //   this.createAdditionalCharges(data.AdditionalChargeDetails)
  ...
  ValidRate(i, evt) {
    ...
    evt.keyCode != 46
      ...
      evt.preventDefault();
      this.Items[this.ItemIndex].PurchaseRate = this.fixPurchaseRate
    } else {
      this.PurchaseRate = Number(evt.target.value);
    }
  }
  fixPurchaseRate:any
  ...
  closeModal() {
    if ($('#Purchase_Return_model').length > 0) {
      $('#Purchase_Return_model').modal(UIConstant.MODEL_HIDE)
    }
    this.showHidePayment = true
  }
  ...
  addItemBasedOnIndustry() {
    ...
    this.Items.push({
      ...
      fixPurchaseRate : +this.fixPurchaseRate,
  ...
  editItem(i, editId, type, sno) {
    ...
    if (type === 'items' && editId > 0) {
     ...
     this.fixPurchaseRate = this.Items[i].fixPurchaseRate
  ...
  private purchaseReturnAddParams(): PurchaseReturnAdd {
    ...
    const saleReturnItem = {
      obj: {
        ...
        PurchaseId: this.Id ? this.Id : UIConstant.ZERO,
  ...
  savePurchaseReturn() {
    ...
    let dataToSend = this.purchaseReturnAddParams()
    let valid = 1
    if (!this.editMode) {
