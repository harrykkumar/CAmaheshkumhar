...
export class serviceBillingService {
  ...
  postPurchaseList (obj) {
    return this.baseService.postRequest(ApiConstant.SERVICE_BILLING_API, obj)
  }
  ...
  createItems (items) {
    let newData = [{ id: '0', text: 'Select Service' }, { id: '-1', text: UIConstant.ADD_NEW_OPTION }]
    items.forEach(item => {
      newData.push({
        id: item.Id,
        text: item.Name
  ...
  createVendors (array) {
    // console.log(array)
    let newData = [{ id: '0', text: 'Select Customer' }, { id: '-1', text: UIConstant.ADD_NEW_OPTION }]
    array.forEach(data => {
      newData.push({
        id: data.Id,
        text: data.Name
  ...
  createOrganisations (array) {
    ...
    getGSTTypeOfOrgnazation (orgDetails) {
        return orgDetails[0].GstTypeId
    }
  ...
  postServiceBilling (obj) {
    return this.baseService.postRequest(ApiConstant.SERVICE_BILLING_API, obj)
  }

  getNewBillNoSaleService (orgId, date, type) {
    let queryString = ''
    if (type === 1) {
  ...
  getServiceBillingEditApi (id): Observable<ResponseSale> {
    return this.baseService.getRequest(ApiConstant.SERVICE_BILLING_API_BY_EDIT + id)
  }
